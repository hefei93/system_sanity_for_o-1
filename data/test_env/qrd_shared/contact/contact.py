...
class Contact(Base):
    ...
    def go_home(self):
        ...
        """
        for i in range(10):
            flag1 = is_view_enabled_by_text(VIEW_TEXT_VIEW, 'Favorites|FAVORITES', isScrollable=0, searchFlag=TEXT_MATCHES)
            flag2 = is_view_enabled_by_text(VIEW_TEXT_VIEW, 'All contacts|ALL', isScrollable=0, searchFlag=TEXT_MATCHES)
            flag3 = is_view_enabled_by_text(VIEW_TEXT_VIEW, 'Groups|GROUPS', isScrollable=0, searchFlag=TEXT_MATCHES)
    ...
    def add_contact_to_phone(self, name, number):#c_caijie
        ...
        if search_view_by_id('floating_action_button'):
            click_button_by_id('floating_action_button')
            sleep(3)
            if search_text('CANCEL',isScrollable=0):
                click_textview_by_text('CANCEL')
        ...
        sleep(3)
        #entertext_edittext_by_index(0, name)
        click_textview_by_text('First name')
        sleep(2)
        os.system('adb shell input text %s'%name)
        send_key(KEY_ENTER)
        sleep(10)
        click_textview_by_text('Phone')
        sleep(2)
        os.system('adb shell input text %s'%number)
        #entertext_edittext_by_index(1, number)
        sleep(10)


        # click btn done
        click_textview_by_id('editor_menu_save_button')
        sleep(5)
        if search_view_by_id('permission_allow_button'):
                click_button_by_id('permission_allow_button')
                sleep(5)
    ...
    def del_all_contact(self):#c_caijie
        ...
        if search_text('Select all',isScrollable=0):
            click_textview_by_text('Select all')
        if search_text('Unselect all', isScrollable=0):
            send_key(KEY_BACK)
        sleep(5)
        click_imageview_by_desc('More options')
        sleep(3)
    ...
    def checkHasContact(self):
        if search_text('ALL'):
            click_textview_by_text('ALL')
            if search_text('No contacts',isScrollable=0):
                return False
            return True
        else:
            log_test_framework('contact', 'maybe is not in contact page')
            return False

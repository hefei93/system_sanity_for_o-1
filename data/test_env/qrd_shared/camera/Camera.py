...
class Camera(Base):
    ...
    #share picture via mms
    def sharePictureMMS(self,number='15629036631'):
        if wait_for_fun(lambda:search_view_by_id('preview_thumb'), True, 5):
            ...
            if wait_for_fun(lambda:search_view_by_id("photopage_bottom_control_delete"), True, 5):
                click_textview_by_id('photopage_bottom_control_share')
                if wait_for_fun(lambda:search_text('Messaging', isVerticalList=0), True, 5):
                    click_textview_by_text('Messaging')
                    sleep(3)
                    if wait_for_fun(lambda:search_view_by_id('recipients_editor'),True,5):
    ...
    #open continuous Off/On          
    def setContinuous(self,maction='Off'):
        if wait_for_fun(lambda:search_view_by_id('menu'),True,5):
            click_textview_by_id('menu')
            if wait_for_fun(lambda:search_text('Continuous Shot'),True,5):
                click_textview_by_text('Continuous Shot')
                sleep(2)
                if maction=='On':
    ...
    #open Gps Off/On
    def setGps(self,maction='Off'):
        if wait_for_fun(lambda:search_view_by_id('menu'),True,5):
            click_textview_by_id('menu')
            if wait_for_fun(lambda:search_text('GPS location'),True,5):
                click_textview_by_text('GPS location')
                sleep(2)
                if maction=='On':
                    click_textview_by_index(21)
                elif maction=='Off':
    ...
    def setSports(self):
        if wait_for_fun(lambda:search_view_by_id("scene_mode_switcher"),True,5):
            ...
            sleep(2)
            click_textview_by_text('Sports',isScrollable=1)
            return True
        take_screenshot()
        return False

...
class Browser(Base):
       ...
       # phone.permission_allow()
        ...
        if search_view_by_id('url'):
           ...
           # entertext_edittext_by_id('url','http://www.baidu.com')
            os.system('adb shell input text http://www.baidu.com')
            sleep(10)
            send_key(KEY_ENTER)
            sleep(60)
    ...
    def browser_downloadQQ(self,isContinue=False):  #go to baidu download QQ and wechat
        self.browser_baidu()
        if wait_for_fun(lambda:search_text("ALLOW",isScrollable=0), True, 30):
            click_textview_by_text("ALLOW")
            if wait_for_fun(lambda:search_view_by_id("permission_allow_button"),True,5):
                click_textview_by_id("permission_allow_button")
                sleep(5)
        if wait_for_fun(lambda:search_text("百度一下"),True, 30):
            sleep(2)
            click_textview_by_id("index-kw")
            sleep(1)
            #entertext_edittext_by_id("index-kw", "QQ")
            os.system('adb shell input text QQ')
            send_key(KEY_ENTER)
            if wait_for_fun(lambda:search_view_by_desc("下载"),True,60):
                click_textview_by_desc("下载")
            if wait_for_fun(lambda:search_view_by_desc("普通下载"),True,60):
                click_textview_by_desc("普通下载")
                if wait_for_fun(lambda:search_text('UPDATE PERMISSIONS',isScrollable=0), True, 5):
                    click_textview_by_text('UPDATE PERMISSIONS') 
                    sleep(3)
                    if wait_for_fun(lambda:search_view_by_id('permission_allow_button'), True, 5):
                        phone.permission_allow()
            if wait_for_fun(lambda:search_view_by_id("button_primary"),True,5):
                click_button_by_id("button_primary")
                if wait_for_fun(lambda:search_view_by_id("button_primary"),True,5):
                    click_button_by_id("button_primary")
                if isContinue:
                    sleep(3)
                    ...
                    click_textview_by_id("index-kw")
                    sleep(1)
                    os.system('adb shell input text weixin')
                    #entertext_edittext_by_id("index-kw", "Weixin")
                    send_key(KEY_ENTER)
                    if wait_for_fun(lambda:search_view_by_desc("下载"),True,60):
                        click_textview_by_desc("下载")
                    if wait_for_fun(lambda:search_view_by_desc("普通下载"),True,60):
                        click_textview_by_desc("普通下载")
                        if wait_for_fun(lambda:search_view_by_id("button_primary"),True,15):
                            click_button_by_id("button_primary")
                        if wait_for_fun(lambda:search_view_by_id("button_primary"),True,5):
                            click_button_by_id("button_primary")
                        sleep(180)

...
class test_suit_gp_sanity_settings_case8(TestCaseBase):
    '''
...
#             phone.permission_allow()                  
        if wait_for_fun(lambda:search_text('WLAN'), True, 5) or wait_for_fun(lambda:search_text('Bluetooth'), True, 5):
            log_test_framework("gp_sanity_settings_case8:", "Launch settings pass")
            settings.tethering_hotspot()
            if wait_for_fun(lambda:search_text("WLAN hotspot"), False, 3):
                log_test_framework("gp_sanity_settings_case8:", "case pass")
                case_flag = True
        elif search_text("stopped", searchFlag=TEXT_CONTAINS):
            log_test_framework("gp_sanity_settings_case8:", "Popup has stopped")
            take_screenshot()
            if search_text("OK"):
            ...
            log_test_framework("gp_sanity_settings_case8:", "case fail")        
        send_key(KEY_BACK)
        sleep(1)
        send_key(KEY_BACK)
        sleep(1)

...
class test_suit_gp_sanity_settings_case6(TestCaseBase):
    '''
...
#             phone.permission_allow()                  
        if wait_for_fun(lambda:search_text('WLAN'), True, 5) or wait_for_fun(lambda:search_text('Bluetooth'), True, 5):
            log_test_framework("gp_sanity_settings_case6:", "Launch settings pass")
            settings.SIM_status()
            if wait_for_fun(lambda:search_text("Network"), True, 3) or search_text("Signal strength"):
                log_test_framework("gp_sanity_settings_case6:", "case pass")
                case_flag = True
        elif search_text("stopped", searchFlag=TEXT_CONTAINS):
            log_test_framework("gp_sanity_settings_case6:", "Popup has stopped")
            take_screenshot()
            if search_text("OK"):
            ...
            log_test_framework("gp_sanity_settings_case6:", "case fail")        
        send_key(KEY_BACK)
        sleep(1)
        send_key(KEY_BACK)
        sleep(1)

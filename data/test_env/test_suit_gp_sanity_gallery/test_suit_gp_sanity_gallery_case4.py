...
class test_suit_gp_sanity_gallery_case4(TestCaseBase):
    '''
...
#             phone.permission_allow()
        if wait_for_fun(lambda:search_text('Timeline'), True, 5) or wait_for_fun(lambda:search_text('Albums'), True, 5):
            log_test_framework("gp_sanity_gallery_case4:", "Launch gallery pass")          
        take_screenshot()
        ...
        if wait_for_fun(lambda:search_view_by_id("send_button_mms"), True, 60) or search_text("REMOVE") or search_text("Message size limit reached", searchFlag=TEXT_CONTAINS):          
            log_test_framework("gp_sanity_gallery_case4:", "Share with mesaging pass")
            case_flag = True       
        if search_text('has stopped'):
            log_test_framework("gp_sanity_gallery_case4:", "Popup has stopped")
            take_screenshot()
            click_textview_by_text('OK')
            sleep(2)
        else:
            log_test_framework("gp_sanity_gallery_case4:", "Share with mesaging fail")
        if search_text("OK"):
            click_textview_by_text('OK')
            sleep(3)
        send_key(KEY_BACK)

...
class test_suit_gp_sanity_gallery_case5(TestCaseBase):
    '''
...
#             phone.permission_allow()
        if wait_for_fun(lambda:search_text('Timeline'), True, 5) or wait_for_fun(lambda:search_text('Albums'), True, 5):
            log_test_framework("gp_sanity_gallery_case5:", "Launch gallery pass")         
        gallery.edit_with_vintage()           
        if search_text('stopped')==False:            
            log_test_framework("gp_sanity_gallery_case5:", "Edit with vintage pass")
            case_flag = True       
        if search_text('has stopped'):
            log_test_framework("gp_sanity_gallery_case5:", "Popup has stopped")
            take_screenshot()
            click_textview_by_text('OK')
            sleep(2)
        else:
            log_test_framework("gp_sanity_gallery_case5:", "Edit with vintage fail")

...
class test_suit_system_sanity_camera_case05(TestCaseBase):
    '''
            ...
            if wait_for_fun(lambda:search_view_by_id('preview_thumb'), True, 5):
                ...
                if wait_for_fun(lambda:search_view_by_id("photopage_bottom_control_delete"), True, 5):
                    ...
                    for i in range(iterationNum):
                        ...
                        log_test_framework(self.name, 'from right to left iteration %d'%(i+1))
                        success_flag = False
                    ...
                    '''
                    for i in range(iterationNum):
                        print_log_line('from left to right iteration %d'%(i+1))
                        log_test_framework(self.name, 'from left to right iteration %d'%(i+1))
                        success_flag = False

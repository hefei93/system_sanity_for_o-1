...
class test_suit_system_sanity_camera_case16(TestCaseBase):
    '''
    ...
    step2.turn on Continuous shooting function.
    step3:take picture with continuous mode
    Verification: 
    ER4: camera continuous taken successfully
    '''
    ...
    def test_case_main(self, case_results):
        ...
        if search_text("Close app", searchFlag=TEXT_CONTAINS):
            ...
            sleep(2) 
        os.system('adb shell pm clear org.codeaurora.snapcam')
        sleep(5)
        ...
        if wait_for_fun(lambda:search_view_by_id('shutter_button'), True, 5):
            ...
            camera.setContinuous('On')
            if wait_for_fun(lambda:search_view_by_id('id/shutter_button'),True,6):
                os.system('adb shell input swipe %s %s %s %s 5000'%(x/2,y-150,x/2+1,y-151))
                after=getCameraCount()
                if after<before:
                    case_flag=True
                    camera.setContinuous('Off')
                    sleep(3)

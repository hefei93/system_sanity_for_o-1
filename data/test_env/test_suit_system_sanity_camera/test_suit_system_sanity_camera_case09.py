...
class test_suit_system_sanity_camera_case09(TestCaseBase):
    '''
    ...
    step2.set picture quality is low->standard->high
    step3.take a picture
    Verification: 
    ER4:the size of captured picture in high lager than standard than low
    '''
    ...
    def test_case_main(self, case_results):
        ...
        if wait_for_fun(lambda:search_view_by_id('shutter_button'), True, 5):
            ...
            switchCR('Picture')
            setPictureQuality('Low')
            takePhoto(1)
            low1=getPictureSize()
            log_test_framework("low quality size:", str(low1))
            setPictureQuality('Standard')
            takePhoto(1)
            sleep(5)
            standard1=getPictureSize()
            log_test_framework("standard quality size:", str(standard1))
            setPictureQuality('High')
            takePhoto(1)
            sleep(5)
            high1=getPictureSize()
            log_test_framework("high quality size:", str(high1))
            if high1>standard1>low1:
                case_flag=True

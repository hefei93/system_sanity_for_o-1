...
class test_suit_system_sanity_camera_case06(TestCaseBase):
    '''
    ...
    '''
    "Procedure:
    Step1: Launch camera and do snapshot with random times
    Step2: Slide to gallery and review the photos
    Step3:Slide back to camera
    Step4:Select setting -> random feature check. 
    Step5:Exit the camera
    Verification: 
    ER4:No exception.    
    '''
    ...
    def test_case_main(self, case_results):
        ...
        global case_flag , TAG, randomCount,recoreName,p 
        recordName=''.join(__name__.split('_')[-2:])
        case_flag = False
        randomCount=random.randint(5,10)
        ...
        if wait_for_fun(lambda:search_view_by_id('shutter_button'), True, 5):
            ...
            if wait_for_fun(lambda:search_view_by_id('preview_thumb'), True, 5):
                ...
                if wait_for_fun(lambda:search_view_by_id("photopage_bottom_control_delete"), True, 5):
                    ...
                    send_key(KEY_BACK)
                    sleep(2)
                    if wait_for_fun(lambda:search_view_by_id('menu'),True,4):

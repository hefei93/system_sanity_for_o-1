...
class test_suit_system_sanity_camera_case01(TestCaseBase):
    '''
    ...
    Step2:Switch to front camera;
    Step7:Repeat step1 to step2 for 30 time;
    Verification: 
    ER7:No exception;    
    '''
    ...
    def test_case_main(self, case_results):
        ...
        """
        if search_text("Close app",isScrollable=0,searchFlag=TEXT_CONTAINS):
            take_screenshot()
        ...
        if wait_for_fun(lambda:search_view_by_id('shutter_button'), True, 5):
            ...
            for i in range(iterationNum):
                ...
                if wait_for_fun(lambda:search_view_by_id('front_back_switcher'), True, 10):
                    log_test_framework("step2:", "Switch to use rear camera pass")               
                    if wait_for_fun(lambda:search_view_by_id("front_back_switcher"), True, 10):

...
class test_suit_system_sanity_camera_case15(TestCaseBase):
    '''
    ...
    step2.Take picture
    Step3:share picture via messaging
    Verification: 
    ER4:can share picture via message successfully
    '''
    ...
    def test_case_main(self, case_results):
        ...
        if wait_for_fun(lambda:search_view_by_id('shutter_button'), True, 5):
            ...
            takePhoto(3)
            camera.sharePictureMMS('15629036631')
            if wait_for_fun(lambda:search_view_by_id('android:id/alertTitle'),True,5):
                click_textview_by_id('android:id/button1')
                sleep(2)
            if wait_for_fun(lambda:search_text('Now'),True,20):

...
class test_suit_system_sanity_camera_case07(TestCaseBase):
    '''
    ...
    step2.select Countdown timer of setting menu
    step3.select Countdown timer to 10s
    step4.Click capture button
    step5.press the power key twice quickly
    Verification: 
    ER4:No exception.    
    '''
    ...
    def test_case_main(self, case_results):
        ...
        TAG = "Press Power key during count down: Camera "
        ...
        if wait_for_fun(lambda:search_view_by_id('shutter_button'), True, 5):
            ...
            case_flag1=setCountDown('10 seconds')
            sleep(2)
            send_key(KEYCODE_POWER)
            sleep(1)
            send_key(KEYCODE_POWER)             
            if not wait_for_fun(lambda:search_view_by_id('count_down_title'), True, 5):
                case_flag=True
            setCountDown('Off')                              

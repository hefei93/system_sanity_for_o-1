...
class test_suit_gp_sanity_camera_case8(TestCaseBase):
    '''
...
#             click_textview_by_text('OK')
        ...
        if search_view_by_id('shutter_button'):
            log_test_framework("gp_sanity_camera_case8:", "Switch video pass")
            click_button_by_id('shutter_button')
            if wait_for_fun(lambda:search_view_by_id('recording_time'), True, 10):
                log_test_framework("gp_sanity_camera_case8:", "Front camera take video pass")            
                case_flag = True
            elif search_text('has stopped'):
                log_test_framework("gp_sanity_camera_case8:", "Front camera take video then camera has stopped")
                take_screenshot()
                click_textview_by_text('OK')
                sleep(2)
        elif search_text('has stopped'):
            log_test_framework("gp_sanity_camera_case8:", "Camera has stopped")
            take_screenshot()
            click_textview_by_text('OK')
            sleep(2)        
                ...
                sleep(5)
                click_textview_by_text("OK")
                log_test_framework("gp_sanity_camera_case8:", "ANR")
        else:
            log_test_framework("gp_sanity_camera_case8:", "Front camera take video fail")
        send_key(KEY_BACK)        
        sleep(1)
        send_key(KEY_BACK)
        sleep(1)

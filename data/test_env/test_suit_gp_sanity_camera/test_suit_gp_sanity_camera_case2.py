...
class test_suit_gp_sanity_camera_case2(TestCaseBase):
    '''
    ...
    def test_case_main(self, case_results):
        ...
        if wait_for_fun(lambda:search_view_by_id('shutter_button'), True, 5):
            log_test_framework("gp_sanity_camera_case2:", "Launch camera pass")
            camera.take_picture()
            if wait_for_fun(lambda:search_view_by_id('shutter_button'), True, 10):            
                case_flag = True
            if search_text('has stopped'):
                log_test_framework("gp_sanity_gallery_case2:", "Take photo then camera has stopped")
                take_screenshot()
                click_textview_by_text('OK')
                sleep(2)
        if search_text('has stopped'):
            log_test_framework("gp_sanity_camera_case2:", "Camera has stopped")
            take_screenshot()
            click_textview_by_text('OK')
            sleep(2)
        else:
            log_test_framework("gp_sanity_camera_case2:", "Take photo fail")
        send_key(KEY_BACK)        
        sleep(1)
        send_key(KEY_BACK)
        sleep(1)

...
class test_suit_gp_sanity_camera_case3(TestCaseBase):
    '''
...
#             click_textview_by_text('OK')
        if wait_for_fun(lambda:search_view_by_id('shutter_button'), True, 10):
            log_test_framework("gp_sanity_camera_case3:", "Launch camera pass")
            ...
            if wait_for_fun(lambda:search_view_by_desc("Switch to video"), True, 5):
                ...
                log_test_framework("gp_sanity_camera_case3:", "Switch to video has stopped")
                click_textview_by_text('OK')
                sleep(2)            
            if search_view_by_id('shutter_button'):
                ...
                if wait_for_fun(lambda:search_view_by_id('recording_time'), True, 10):                    
                    log_test_framework("gp_sanity_camera_case3:", "Take video pass")
                    click_button_by_id('shutter_button')
                    case_flag = True            
        elif search_text('has stopped'):
            log_test_framework("gp_sanity_camera_case3:", "Camera has stopped")
            take_screenshot()
            click_textview_by_text('OK')
            sleep(2)        
        else:
            log_test_framework("gp_sanity_camera_case3:", "Take video fail")                
        click_imageview_by_id('camera_switcher')

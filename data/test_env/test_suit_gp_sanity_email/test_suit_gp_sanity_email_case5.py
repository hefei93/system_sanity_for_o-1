...
class test_suit_gp_sanity_email_case5(TestCaseBase):
    '''
...
#             phone.permission_allow()
        ...
        if search_text('Exchange')==False:
            log_test_framework("gp_sanity_email_case5:", "Remove email account pass")
            case_flag = True
        elif search_text('has stopped'):
            log_test_framework("gp_sanity_email_case5:", "Popup has stopped")
            take_screenshot()
            ...
            log_test_framework("gp_sanity_email_case5:", "Remove account fail")
            take_screenshot()
        send_key(KEY_BACK)
        sleep(1)
        send_key(KEY_BACK)

...
class test_suit_gp_sanity_email_case2(TestCaseBase):
    '''
    ...
    def test_case_main(self, case_results):
        ...
        if wait_for_fun(lambda:search_text('Account setup'), True, 5):
            log_test_framework("gp_sanity_email_case2:", "Launch email pass")
            ...
            if wait_for_fun(lambda:search_text('Exchange'), True, 5) or search_text("Personal (IMAP)"):
                log_test_framework("gp_sanity_email_case2:", "Create email account pass")
                case_flag = True
        if search_text('has stopped'):
            log_test_framework("gp_sanity_email_case2:", "Popup has stopped")
            take_screenshot()
            click_textview_by_text('OK')
            sleep(2)    
        else:
            log_test_framework("gp_sanity_email_case2:", "Create email account fail")
            take_screenshot()

...
class test_suit_gp_sanity_email_case4(TestCaseBase):
    '''
...
#             phone.permission_allow()
        if wait_for_fun(lambda:search_text('Inbox'), True, 5):
            log_test_framework("gp_sanity_email_case4:", "Launch email account pass")
        ...
        if wait_for_fun(lambda:search_view_by_id("empty_text"), False, 120):
            log_test_framework("gp_sanity_email_case4:", "Search email people pass")
        ...
        send_key(KEY_ENTER)               
        if wait_for_fun(lambda:search_view_by_id("empty_text"), False, 120):
            log_test_framework("gp_sanity_email_case4:", "Search email subject pass")
            case_flag_subject = True
        case_flag=case_flag_people and case_flag_subject                
        if search_text('has stopped'):
            log_test_framework("gp_sanity_email_case4:", "Popup has stopped")
            take_screenshot()
            ...
            log_test_framework("gp_sanity_email_case4:", "Search email fail")
            take_screenshot()
        send_key(KEY_BACK)
        sleep(2)
        send_key(KEY_BACK)

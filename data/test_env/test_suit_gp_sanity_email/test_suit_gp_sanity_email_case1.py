...
class test_suit_gp_sanity_email_case1(TestCaseBase):
    '''
    ...
    def test_case_main(self, case_results):
        ...
        def execute():
            ...
            if wait_for_fun(lambda:search_text('Account setup'), True, 5):
                log_test_framework("gp_sanity_email_case1:", "Launch email pass")
                send_key(KEY_BACK)
                send_key(KEY_BACK)
                send_key(KEY_HOME)
                if wait_for_fun(lambda:search_view_by_desc("Apps list"), True, 3):
                    log_test_framework("gp_sanity_email_case1:", "Exit pass")
                    case_flag = True
            if search_text('has stopped'):
                log_test_framework("gp_sanity_email_case1:", "Popup has stopped")
                take_screenshot()
                if search_text("OK"):
                ...
                log_test_framework("gp_sanity_email_case1:", "Launch email fail")       

...
class test_suit_gp_sanity_message_case10(TestCaseBase):
    '''
    ...
    def test_case_main(self, case_results):
        ...
        if search_text("^\d\dK/300K$", searchFlag=TEXT_MATCHES_REGEX):
            log_test_case("test_suit_gp_sanity_message_case10", "attach first attachment successfully")
            take_screenshot()
        else:
            take_screenshot()
            log_test_case("test_suit_gp_sanity_message_case10", "Fail to attach first attachment")
            set_cannot_continue()
        sleep(3)
        ...
        if not wait_for_fun(func_size, True, 10):
            log_test_case("test_suit_gp_sanity_message_case10", "can not found the size of video")
        ...
        if search_text("^\d\d+K/300K$", searchFlag=TEXT_MATCHES_REGEX)and(not attach_size1==attach_size2):
            log_test_case("test_suit_gp_sanity_message_case10", "Replace attachment successfully")
            take_screenshot()
        else:
            take_screenshot()
            log_test_case("test_suit_gp_sanity_message_case10", "Fail to replace first attachment")
            set_cannot_continue()
        
        func2 = lambda:search_text('MMS', isScrollable=1, searchFlag=TEXT_CONTAINS) 
        if not wait_for_fun(func2, True, 10):
            set_cannot_continue()
            log_test_framework("gp_sanity_message_case10:", "send button not found")
        else:
        ...
        if not wait_for_fun(func3, True, 120):
            log_test_framework("gp_sanity_message_case10:", "Sent mms failed")
        else:
            log_test_framework("gp_sanity_message_case10:", "Sent mms successful")
            case_flag = True

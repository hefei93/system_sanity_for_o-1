...
class test_suit_gp_sanity_gallery_case3(TestCaseBase):
    '''
...
#             phone.permission_allow()
        if wait_for_fun(lambda:search_text('Timeline'), True, 5):
            log_test_framework("gp_sanity_gallery_case3:", "Launch gallery pass")      
        ...
        if wait_for_fun(lambda:search_view_by_id('shutter_button'), True, 5):            
            log_test_framework("gp_sanity_gallery_case3:", "Go into camera pass")
            case_flag = True       
        elif search_text('has stopped'):
            log_test_framework("gp_sanity_gallery_case3:", "Popup has stopped")
            take_screenshot()
            click_textview_by_text('OK')
            sleep(2)
        else:
            log_test_framework("gp_sanity_gallery_case3:", "Go into camera fail")
        send_key(KEY_BACK)
        sleep(1)
        send_key(KEY_BACK)
        sleep(1)
